...
def train_test(data_folder, view_list, num_class,
    ...
    if data_folder == './artifacts/data_preprocessing/BRCA' or data_folder == './artifacts/data_preprocessing/KIPAN' or data_folder == './artifacts/data_preprocessing/BLCA':
        adj_parameter = 8
    ...
    for epoch in range(num_epoch+1):
        ...
        if epoch % test_inverval == 0:
            ...
            if num_class == 2:
                auc_macro = roc_auc_score(labels_trte[trte_idx["te"]] , te_prob[:,1] , average='macro' , multi_class='ovr')
                auc_micro = roc_auc_score(labels_trte[trte_idx["te"]] , te_prob[:,1] , average='micro' , multi_class='ovr')
                auc_weighted = roc_auc_score(labels_trte[trte_idx["te"]] , te_prob[:,1] , average='weighted' , multi_class='ovr')
            else:
